...
export function Produtos() {
  ...
  function obterNome() {
    axios
      .get(`http://localhost:3001/produtos/busca?nome=${getNome}`)
      .then((response) => {
  ...
  function obterCategoria() {
    axios
      .get(`http://localhost:3001/produtos/busca?categoria=${getCategoria}`)
      .then((response) => {
        setResultadosBusca(response.data);
      })
      .catch((error) => {
  ...
  return (
    <div className="container">
        ...
        <Form className="d-flex flex-row w-50">
          <div className="d-flex  justify-content-center align-items-center me-2 h-10">
            <InputGroup>
              ...
              />
              <Button onClick={obterNome} variant="outline-success">
                Busca Nome
              </Button>
            </InputGroup>
          </div>
          ...
          <div className="d-flex  justify-content-center align-items-center">
            <InputGroup>
              ...
              />
              <Button onClick={obterCategoria} variant="outline-success">
                Busca Categoria
              </Button>
            </InputGroup>
          </div>
      ...
      <div className="d-flex justify-content-between align-items-center mt-2 mb-1">
        <h1>Produtos</h1>
        <Button as={Link} to="/produtos/novo" className="btn btn-primary">
            ...
            produtos.length > 0 ? (
              resultadosBusca !== null && resultadosBusca.length > 0 ? (
                resultadosBusca.map((produto) => { 
                  //Renderizar tabela com resultado pesquisa
                  return (
                    <tr key={produto.id} >
                      <td >{produto.nome}</td>
                      ...
                      <td>
                        {format(new Date(produto.dataDesconto), "yyyy-MM-dd")}
                      </td>
                ...
                produtos.map((produto) => {
                  //Renderizar tabela com  todos os produtos
                  return (
                    <tr key={produto.id}>
                      <td>{produto.nome}</td>
                      <td>R$ {produto.preco}</td>
                      ...
                      <td>{produto.desconto * 100}%</td>
                      <td>
                        {new Date(produto.dataDesconto).toLocaleDateString()}
                      </td>
                      <td>{produto.categoria}</td>
                      <td>{produto.id}</td>
                      <td className="align-middle d-flex gap-2 justify-content-center">
